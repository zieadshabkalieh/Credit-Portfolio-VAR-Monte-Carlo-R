...
for (year in names(data)) {
  ...
  correlation_matrix <- matrix(runif(n_companies * n_companies, min = 0.1, max = 0.9), 
                               nrow = n_companies, ncol = n_companies)
  diag(correlation_matrix) <- 1
  gaussian_copula <- normalCopula(param = correlation_matrix[lower.tri(correlation_matrix)], 
